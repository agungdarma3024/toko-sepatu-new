...
export default function SemuaProdukPage() {
  ...
  // === 2. LOGIKA JAVASCRIPT UNTUK MENYARING DATA BERLAPIS ===
  const filteredProducts = products.filter(product => {
    // Cek Kategori & Gender
    const matchCategory = selectedCategories.length === 0 || selectedCategories.includes(product.category.toLowerCase());
    const matchGender = selectedGenders.length === 0 || selectedGenders.includes(product.gender || 'Unisex');
    
    // Cek Ukuran (Jika produk tidak punya array sizes, anggap sedia 39-42)
    ...
    // Produk harus lolos SEMUA syarat di atas
    return matchCategory && matchGender && matchSize && matchColor && matchPrice;
  });

  // === 3. FUNGSI TOGGLE (ON/OFF) FILTER ===
  ...
  // === 4. KOMPONEN ISI FILTER (DIPAKAI DI HP & LAPTOP) ===
  const FilterContent = () => (
      ...
      <div>
        ...
        <div className="flex flex-col gap-2">
          <label className="flex items-center gap-3 cursor-pointer text-sm text-gray-600">
            <input type="radio" name="price" checked={priceRange === ""} onChange={() => setPriceRange("")} className="w-4 h-4 accent-black" /> Semua Harga
          </label>
          <label className="flex items-center gap-3 cursor-pointer text-sm text-gray-600">
            <input type="radio" name="price" checked={priceRange === "under500"} onChange={() => setPriceRange("under500")} className="w-4 h-4 accent-black" /> Di bawah Rp 500rb
          </label>
          <label className="flex items-center gap-3 cursor-pointer text-sm text-gray-600">
            <input type="radio" name="price" checked={priceRange === "500to1m"} onChange={() => setPriceRange("500to1m")} className="w-4 h-4 accent-black" /> Rp 500rb - Rp 1 Juta
          </label>
          <label className="flex items-center gap-3 cursor-pointer text-sm text-gray-600">
            <input type="radio" name="price" checked={priceRange === "over1m"} onChange={() => setPriceRange("over1m")} className="w-4 h-4 accent-black" /> Di atas Rp 1 Juta
          </label>
        </div>
      </div>
      <div className="w-full h-px bg-gray-100"></div>
        ...
        <div className="flex flex-wrap gap-2">
          {AVAILABLE_SIZES.map(size => (
            <button 
              ...
              className={`w-10 h-10 text-xs font-medium border rounded-md transition-colors ${filterSizes.includes(size) ? 'bg-black text-white border-black' : 'hover:border-black text-gray-600'}`}
            >
              {size}
            </button>
          ))}
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {AVAILABLE_COLORS.map(color => (
            <button 
              key={color} 
              onClick={() => toggleArrayItem(setFilterColors, color)}
              className={`px-3 py-1.5 text-xs font-medium border rounded-md transition-colors ${filterColors.includes(color) ? 'bg-black text-white border-black' : 'hover:border-black text-gray-600'}`}
            >
              {color}
            </button>
          ))}
  ...
  const handleWishlistClick = (e: React.MouseEvent, productId: string) => {
    e.preventDefault(); e.stopPropagation(); toggleWishlist(productId);
  };
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="mb-8 md:mb-10 border-b border-gray-200 pb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-gray-500 mt-2 text-sm md:text-base">Menampilkan {filteredProducts.length} pasang sepatu untuk Anda.</p>
          </div>
                        ...
                        <Card className="group overflow-hidden border-gray-200 hover:shadow-xl transition-all duration-300 relative cursor-pointer h-full flex flex-col rounded-xl md:rounded-2xl bg-white active:scale-[0.98] md:active:scale-100">
                          ...
                          <CardContent className="p-3 md:p-4 flex flex-col flex-grow">
                            <div className="flex justify-between items-center mb-1.5 md:mb-2 text-[10px] md:text-xs text-gray-500">
                              <div className="flex gap-1 items-center"><Star className="h-3 w-3 md:h-3.5 md:w-3.5 fill-yellow-400 text-yellow-400" />{product.rating}</div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2">
                          ...
                          <div className="p-5 md:p-8 flex flex-col h-full overflow-y-auto max-h-[60vh] md:max-h-[90vh]">
                            ...
                            <div className="mb-4">
                              ...
                              <div className="flex flex-wrap gap-2">
                                {(product.sizes || [39,40,41,42]).map(size => (
                                  <button key={size} onClick={() => setSelectedSize(size)} className={`w-9 h-9 md:w-10 md:h-10 text-xs md:text-sm border rounded-md ${selectedSize === size ? 'bg-black text-white border-black' : 'hover:border-black'}`}>{size}</button>
                                ))}
                              </div>
                            </div>
                            ...
                            <div className="mb-6">
                              ...
                              <div className="flex flex-wrap gap-2">
                                {(product.colors || ['Hitam', 'Putih']).map(color => (
                                  <button key={color} onClick={() => setSelectedColor(color)} className={`px-3 py-1.5 text-xs md:text-sm border rounded-md ${selectedColor === color ? 'bg-black text-white border-black' : 'hover:border-black'}`}>{color}</button>
                                ))}
                              </div>
                            </div>
                            ...
                            <div className="flex gap-2 md:gap-3 mt-auto pt-4 md:pt-6 border-t border-gray-100">
                              ...
                              <Button className="flex-[3] gap-2 h-10 md:h-12 bg-black hover:bg-gray-800 text-white shadow-md text-xs md:text-sm" onClick={() => handleBeliSekarang(product)}>
                                Beli Sekarang
                              </Button>
                            </div>
                          </div>
